...
export async function loadLightSign() {
  ...
  function buildLine(letters, baselineY, rightEdge = null) {
    const built = letters.map((letter) => {
      const object = originals[letter].clone()
      const box = new THREE.Box3().setFromObject(object)
      const size = box.getSize(new THREE.Vector3())
      const scale = SCALE / Math.max(size.x, size.y, size.z)
      object.scale.setScalar(scale)
      return { object, box, scale, width: size.x * scale }
    })
    ...
    built.forEach(({ object, box, scale, width }, i) => {
      object.position.set(
  ...
  group.hitArea = new THREE.Mesh(
    new THREE.BoxGeometry(signSize.x, signSize.y, Math.max(signSize.z, 10)),
    new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 })
  )
  group.hitArea.position.copy(signBox.getCenter(new THREE.Vector3()))

  group.meshes = getMeshes(group, [group.hitArea])
  group.hoverColor = 0x88ffff
